...
if command -v python3 >/dev/null 2>&1 && python3 -m pip --version >/dev/null 2>&1; then
    ...
    if ! python3 -m pip install "${pip_install_args[@]}" "$HOME/.mio"; then
        echo "Warning: failed to install the optional Python package; ~/.mio/mio is still available." >&2
    fi
else
    echo "Info: pip not found; using the standalone ~/.mio/mio launcher." >&2
fi
